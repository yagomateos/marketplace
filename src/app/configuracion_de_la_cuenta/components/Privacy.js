...
export default function Privacy({ setStep }) {
    ...
    const [personalAdvertising, setPersonalAdvertising] = useState(false)
    const [privacySuccess, setPrivacySuccess] = useState(null)

    const warningHandler = (what, e) => {
        e.preventDefault();
        setShowWarning(what)
    }

    const warningAccepted = (what, e) => {
        e.preventDefault();
        switch (what) {
            case 'recentlyViewed':
                handleRecentAds()
    ...
    return (
        <div className='relative'>
            ...
            <div className="p-4 rounded-lg border border-[#ccc] mb-4">
                ...
                <a onClick={(e) => warningHandler('recentlyViewed', e)} className="p-1 text-center lg:p-2 border-2 border-[#000] rounded-full mt-8 inline-block cursor-pointer">Borrar anuncios vistos recientemente</a>
                {resentAdsSuccess && <div className='mt-3 text-sm text-green-700'>{resentAdsSuccess}</div>}
            </div>
            {/* second box */}
            <div className="p-4 rounded-lg border border-[#0a0909] mb-4">
            ...
            <div className="p-4 rounded-lg border border-[#ccc] mb-4">
                ...
                <a onClick={(e) => warningHandler('deleteDta', e)} className="p-1 text-center lg:p-2 border-2 border-[#000] rounded-full mt-8 inline-block cursor-pointer">Solicitar la eliminación de mis datos</a>
            ...
            <div className="p-4 rounded-lg border border-[#ccc] mb-4">
                ...
                <span className='flex w-full justify-between'><p>¿Quieres que otros puedan ver tu dirección de correo electrónico? Se mostrará públicamente.</p>
                    <input type='radio' name='emailVisible' checked={emailVisible} onClick={(e) => setEmailVisible(!emailVisible)} /></span>
                ...
                <span className='flex w-full justify-between'><p>Usamos cookies y tecnologías similares para mejorar tu experiencia en cuestiones como:</p>
                    <input checked={cookieAllowed} onClick={e => { setCookieAllowed(!cookieAllowed) }} type='radio' /></span>
                <ul className='text-sm mt-3'>
                ...
                <span className='flex w-full justify-between mt-3'><b className='block'>Publicidad personalizada</b>
                    <input checked={personalAdvertising} onClick={e => { setPersonalAdvertising(!personalAdvertising) }} type='radio' />
                </span>
                ...
                <a onClick={(e) => {e.preventDefault(); setPrivacySuccess('¡Configuración guardada exitosamente!')}} className="p-2 border-2 border-[#000] rounded-full mt-8 inline-block cursor-pointer">Actualizar configuración</a>
                {privacySuccess && <div className='mt-5 text-green-700 text-sm'>{privacySuccess}</div>}
            </div>

            {showWarning && <div className='absolute w-full h-full bg-[#ffffffcf] left-0 top-0 text-black py-[60px] flex flex-col items-center'>
                <h2 className='text-4xl text-red-700 w-full text-center'>Estas seguro?</h2>
                <a href='#' onClick={e => warningAccepted(showWarning, e)} className='py-3 px-10 rounded-md bg-black text-white mt-4 inline-block'>confirmar</a>
            </div>}
        </div>
    )
}
