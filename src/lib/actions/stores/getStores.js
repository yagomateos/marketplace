'use server'
import { getStoresById, getStoresByName, getStoresByUserId, getStoresInformation, setDelDates } from '../../middleware/store/getStores'
...
export const getStrId = async (id) => {
    try {
        const stores = await getStoresById(id)
        console.log(stores)
        return stores;
    } catch (error) {
        throw error
